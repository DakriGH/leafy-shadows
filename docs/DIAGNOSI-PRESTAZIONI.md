...
> ⚠ **STATO AL 02/09 — questa è una FOTOGRAFIA di com'era, non di com'è.** La
> diagnosi è stata scritta guardando la build pubblicata de-minificata; il
> lavoro sul sorgente l'ha percorsa nell'ordine proposto al §4 e i numeri
> nuovi stanno in `MISURE.md`. In sintesi:
>
> | punto | stato |
> |---|---|
> | §4.1 ricetta di partenza non-`lago` | **fatto** — si parte da `ghibli` finché le passate non sono a bilancio |
> | §4.2 acqua nei profili + scalatore riattaccato | **fatto** — cinque colonne nuove nella tabella dei livelli, e la scala è automatica finché non la tocchi |
> | §4.3 una passata sola, liste cullate e ripulite, specchio 256² | **fatto** — e unendo le due passate è venuto fuori che la profondità sbagliava di un fattore ~8 |
> | §4.4 ombre desktop, picking | **fatto** — 3 cascate e riduttore min/max spento; il picking, misurato, costava ZERO |
> | §4.5 erba, lampade in UBO | da fare |
> | §4.6 mesher in Worker | da fare |
>
> ⚠ E due punti di questa diagnosi nel SORGENTE non erano veri: il sole è già
> quantizzato in `firmaQuiete` (3 scongelamenti su 600 fotogrammi, misurato) e
> il picking di Babylon al tocco non costa niente perché nessuno glielo chiede
> — e con l'origine mobile il suo raggio è pure degenere. Vedi `MISURE.md`,
